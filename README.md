...
To fit every probe on a single non-scrolling screen, set `mode: matrix` on the display in `config/build-deploy.local.yaml` (the supported modes are `standard` (default), `compact`, and `matrix`). The matrix draws one row per device and one column per probe class — `P` ping, `R` REST/HTTP, `F` FTP, `T` telnet, `I` ident, `D` DMA — with a single glyph per cell: `.` OK, lowercase `x` degraded, `X` failed (reverse video), `?` unknown, and blank when that probe is not configured for the device.
